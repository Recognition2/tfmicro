...
use context::{ElemTypeOf, ElementType, QuantizationParams, TensorInfo};
...
// {
//     fn drop(&mut self) {
//         let handle = core::mem::replace(&mut self.handle, Default::default());
...
//     &self,
//     tensor_index: TensorIndex,
// ) -> Fallible<&bindings::TfLiteTensor> {
//     let interpreter = self.handle();
...
//     type Target = bindings::TfLiteTensor;
...
impl Tensor {
    ...
    //     Ok(unsafe {
    //         slice::from_raw_parts(inner.data.raw_const as *mut u8, inner.bytes)
    //     })
    // }

    // pub fn tensor_buffer_mut(
    //     &mut self,
    //     tensor_index: TensorIndex,
    // ) -> Fallible<&mut [u8]> {
    //     let inner = self.tensor_inner(tensor_index)?;
    ...
    //     Ok(unsafe {
    //         slice::from_raw_parts_mut(inner.data.raw as *mut u8, inner.bytes)
    //     })
    // }
}
...
//             .expect("Not able to build builder");
//         send_sync(&builder);
